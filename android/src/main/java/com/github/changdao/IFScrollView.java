 ...
 *
 * <p>IFScrollView only supports vertical scrolling. For horizontal scrolling,
 * use {@link ReactHorizontalScrollView}.
 */
@TargetApi(11)
public class IFScrollView extends ScrollView implements ReactClippingViewGroup, ViewGroup.OnHierarchyChangeListener, View.OnLayoutChangeListener {
  ...
  private boolean mInnerFirst = false;
  private int mLastXIntercept = 0;
  private int mLastYIntercept = 0;
  private int mIFScrollDirection = 0; //0: undefined, 1:horizontal, 2:vertical
  ...
  public IFScrollView(ReactContext context) {
    this(context, null);
  }

  public IFScrollView(ReactContext context, @Nullable FpsListener fpsListener) {
    super(context);
  ...
  public void setInnerFirst(boolean innerFirst){
    mInnerFirst = innerFirst;
  }
  ...
  protected void onScrollChanged(int x, int y, int oldX, int oldY) {
    ...
    if (mOnScrollDispatchHelper.onScrollChanged(x, y)) {
      ...
      IFScrollViewHelper.emitScrollEvent(
        this,
  ...
  public boolean onInterceptTouchEvent(MotionEvent ev) {
    if (!mScrollEnabled) {
      Log.i("ReactNativeJS","IFScrollView scrollEnabled false");
      return false;
    }

    boolean continueIntercept=false;
    ...
    if(mInnerFirst){
      ...
      switch(ev.getAction()){
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if(mIFScrollDirection==0){
              ...
              if(Math.abs(x-mLastXIntercept)>Math.abs(y-mLastYIntercept)){
                ...
                mIFScrollDirection=2;
              } 
            }
            continueIntercept = (mIFScrollDirection==2);
        ...
        case MotionEvent.ACTION_UP:
            ...
            break;
        default:
            //toto: continueIntercept =  (mIFScrollDirection==2); 
            super.onInterceptTouchEvent(ev);
            continueIntercept = false;
            break;
      }
      ...
      try {
        if (continueIntercept && super.onInterceptTouchEvent(ev)) {
          ...
          IFScrollViewHelper.emitScrollBeginDragEvent(this);
          mDragging = true;
          enableFpsListener();
          return true;
        }
  ...
  public boolean onTouchEvent(MotionEvent ev) {
    ...
    if (action == MotionEvent.ACTION_UP && mDragging) {
      ...
      IFScrollViewHelper.emitScrollEndDragEvent(
        this,
        velocityX,
        velocityY);
      mDragging = false;
  ...
  private void handlePostTouchScrolling(int velocityX, int velocityY) {
    ...
    if (mSendMomentumEvents) {
      enableFpsListener();
      IFScrollViewHelper.emitScrollMomentumBeginEvent(this, velocityX, velocityY);
    }
    ...
    mPostTouchRunnable = new Runnable() {
      ...
      public void run() {
        if (mActivelyScrolling) {
          ...
          mActivelyScrolling = false;
          ViewCompat.postOnAnimationDelayed(IFScrollView.this,
            this,
            IFScrollViewHelper.MOMENTUM_DELAY);
        } else {
          if (mPagingEnabled && !mSnappingToPage) {
            ...
            if (mSendMomentumEvents) {
              IFScrollViewHelper.emitScrollMomentumEndEvent(IFScrollView.this);
            }
            IFScrollView.this.mPostTouchRunnable = null;
            disableFpsListener();
          }
        }
      }
